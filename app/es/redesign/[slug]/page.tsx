...
export async function generateStaticParams() {
  return getRedesigns("es").map((r) => ({ slug: r.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  return redesignPageMetadata("es", slug);
}

export default async function RedesignPage({ params }: Props) {
  const { slug } = await params;
  const page = getRedesignFor("es", slug);
  if (!page) notFound();
  return <RedesignPageView locale="es" page={page} />;
}
